...
type CalloutType = 'idea' | 'info' | 'thought' | 'warning';
...
function CustomLink(props: CustomLinkProps) {
	...
	if (href.startsWith("/")) {
		return (
			<Link className={styles} to={href} {...props}>
				{props.children}
			</Link>
		);
	}
...
const Pre = ({ children, ...props }: React.HTMLAttributes<HTMLPreElement>) => {
	...
	const className = (code?.props as any)?.className || "";
...
function OrderedList({ children }: ChildrenProps) {
	return <ol className="mb-8 list-decimal pl-8">{children}</ol>;
}

function UnorderedList({ children }: ChildrenProps) {
...
function FullWidthCallout({ children, type }: FullWidthCalloutProps) {
