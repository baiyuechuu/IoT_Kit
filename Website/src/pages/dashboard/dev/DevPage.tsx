...
import { Plus, Settings, Trash2, AlertCircle, Loader2, ArrowLeft } from "lucide-react";
...
export default function DevPage() {
	...
	useEffect(() => {
		const loadDashboard = async () => {
			...
			try {
				const { data, error } = await dashboardService.getDashboard(dashboardId);
	...
	useEffect(() => {
		const updateSizing = () => {
			...
			const maxWidth = Math.min(windowWidth * 0.9, 1400); // 90% of window width, max 1400px
			setGridWidth(Math.max(maxWidth - paddingOffset, 800)); // Min width of 800px
	...
	const handleWidgetSave = useCallback((updatedWidget: WidgetConfig) => {
		const updatedWidgets = widgets.map((w) => (w.i === updatedWidget.i ? updatedWidget : w));
		updateWidgets(updatedWidgets);
	}, [widgets, updateWidgets]);
					...
					<div className="flex max-md:flex-col gap-3 sm:gap-4 min-md:items-center">
						...
						<div className="flex gap-2 sm:gap-3">
							...
							<Button
								...
								disabled={!editMode || !isDesktop || widgets.length === 0 || !isReady || widgetsLoading}
								className="text-xs sm:text-sm"
							>
